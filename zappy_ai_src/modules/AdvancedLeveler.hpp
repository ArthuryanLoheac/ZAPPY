...
class AdvancedLeveler : public AIModule {
 public:
    ...
    /// @brief Current action ran by the module.
 ...
 private:
    /**
     * Helper function which computes the priority of the module based
     * on the player's inventory's content.
     */
    double computePriority(int level, const std::unordered_map
        <AI::Data::Material_t, int> &inventory);

    State_t _moduleState;
};
